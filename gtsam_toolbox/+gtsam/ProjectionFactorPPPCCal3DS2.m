...
%string_deserialize(string serialized) : returns ProjectionFactorPPPCCal3DS2
%
classdef ProjectionFactorPPPCCal3DS2 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = ProjectionFactorPPPCCal3DS2(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_unstable_wrapper(492, varargin{2});
        end
        base_ptr = gtsam_unstable_wrapper(491, my_ptr);
      elseif nargin == 6 && isa(varargin{1},'gtsam.Point2') && isa(varargin{2},'gtsam.noiseModel.Base') && isa(varargin{3},'numeric') && isa(varargin{4},'numeric') && isa(varargin{5},'numeric') && isa(varargin{6},'numeric')
        [ my_ptr, base_ptr ] = gtsam_unstable_wrapper(493, varargin{1}, varargin{2}, varargin{3}, varargin{4}, varargin{5}, varargin{6});
      elseif nargin == 8 && isa(varargin{1},'gtsam.Point2') && isa(varargin{2},'gtsam.noiseModel.Base') && isa(varargin{3},'numeric') && isa(varargin{4},'numeric') && isa(varargin{5},'numeric') && isa(varargin{6},'numeric') && isa(varargin{7},'logical') && isa(varargin{8},'logical')
        [ my_ptr, base_ptr ] = gtsam_unstable_wrapper(494, varargin{1}, varargin{2}, varargin{3}, varargin{4}, varargin{5}, varargin{6}, varargin{7}, varargin{8});
      else
    ...
    function delete(obj)
      gtsam_unstable_wrapper(495, obj.ptr_gtsamProjectionFactorPPPCCal3DS2);
    end
    ...
    function varargout = string_serialize(this, varargin)
      ...
      if length(varargin) == 0
        varargout{1} = gtsam_unstable_wrapper(499, this, varargin{:});
      else
        error('Arguments do not match any overload of function gtsam.ProjectionFactorPPPCCal3DS2.string_serialize');
      end
    end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      % STRING_DESERIALIZE usage: string_deserialize() : returns gtsam.ProjectionFactorPPPCCal3DS2
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      if length(varargin) == 1
        varargout{1} = gtsam_unstable_wrapper(500, varargin{:});
      else
        error('Arguments do not match any overload of function gtsam.ProjectionFactorPPPCCal3DS2.string_deserialize');
      end
    end

    function obj = loadobj(sobj)
      % LOADOBJ Saves the object to a matlab-readable format
      obj = gtsam.ProjectionFactorPPPCCal3DS2.string_deserialize(sobj);
    end
  end
end
